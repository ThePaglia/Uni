...
void drawCircle(float cx, float cy, float rx, float ry, Vertex *circle)
{
	...
	for (int i = 0; i <= nVertices; i++)
	{
		...
		circle[i].x = cx + rx * (0.5 - cos(2 * theta_i) - cos(theta_i));
		circle[i].y = cy + ry * (3 * sin(theta_i));
		circle[i].z = 0;
		circle[i].r = 1.0f;
		circle[i].g = 1.0f;
		circle[i].b = 1.0f;
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("Luna OpenGL");			  // Crea una finestra sullo schermo e gli d� un titolo
