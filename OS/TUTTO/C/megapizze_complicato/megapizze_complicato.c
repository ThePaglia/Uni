...
/* da chiamare in mutua esclusione */
int PreparaPortaPizzeEAvvisa( char *Plabel ) {
	/* prepara pizza senza perdere tempo*/

	/* poi porta la pizza in tavola, qui sono gia' in mutua esclusione  */
...
{ 
	...
	while(1) {
		...
		if( pizzaiolochiamato > 0 ) {
			PreparaPortaPizzeEAvvisa( Plabel );	/* qui faccio unlock mentre preparo e lock dopo */
		}
		else {
...
{ 
	...
	*intptr=0;
	rc=pthread_create( &th,NULL,Pizzaiolo,(void*)intptr); 
