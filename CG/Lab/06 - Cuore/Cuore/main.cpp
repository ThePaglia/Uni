...
void drawCircle(float cx, float cy, float rx, float ry, Vertex *circle)
{
	...
	for (int i = 0; i <= nVertices; i++)
	{
		...
		circle[i].x = cx + rx * (16 * pow(sin(theta_i), 3));
		circle[i].y = cy + ry * (13 * cos(theta_i) - 5 * cos(2 * theta_i) - 2 * cos(3 * theta_i) - cos(4 * theta_i));
		circle[i].z = 0;
		circle[i].r = 1.0f;
		circle[i].g = 0.0f;
		circle[i].b = 0.0f;
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("Cuore OpenGL");			  // Crea una finestra sullo schermo e gli d� un titolo
