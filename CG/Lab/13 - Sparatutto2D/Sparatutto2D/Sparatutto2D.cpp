...
void InterpolazioneHermite(float* t, Forma* Fig, vec4 color_top, vec4 color_bot)
{
	...
	Fig->vertici.clear();
	Fig->colors.clear();
	Fig->vertici.push_back(vec3(0.0, 0.5, 0.0)); // Aggiungo il vertice iniziale centrale da dove partono i triangoli
...
void costruisci_proiettile(float cx, float cy, float raggiox, float raggioy, Forma* fig)
{
	...
	{
		t = (float)i * stepA;
		fig->vertici.push_back(vec3(cx + raggiox * cos(t) + 4, cy + raggioy * sin(t), 0.0));
		// Colore
		fig->colors.push_back(vec4(1.0, 204.0 / 255.0, 0.0, 1.0));
	}
...
void crea_punti_forma_da_file()
{
	...
	while (fscanf(file, "%f %f %f", &dati[riga].x, &dati[riga].y, &dati[riga].z) == 3)
	{
		...
		if (riga >= 1000)
		{
			printf("Troppe righe nel file. L'array dati e' stato completamente riempito.\n");
			break;
		}
	}
...
void INIT_VAO(void)
{
	...
	costruisci_proiettile(1.0, 0.0, 1.0, 1.0, &Proiettile);
...
void drawScene(void)
{
	...
	Scena[2].Model = scale(Scena[2].Model, vec3(1.5f, 1.0f, 1.0f));

	/*Matrice di modellazione del proiettile*/

	Scena[3].Model = translate(Scena[2].Model, vec3(posx_Proiettile - 120, posy_Proiettile + 120, 0.0));
	...
	if (Scena[1].alive == TRUE)
	{
		...
		RenderText(programId_text, Projection, "Spara ai nemici", VAO_Text, VBO_Text, 100.0f, 600.0f, 1.0f, glm::vec3(1.0, 1.0f, 0.2f));
		glBindVertexArray(0);
	}
	else
