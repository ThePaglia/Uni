...
int numAereiRichiedentiAtterraggio;
...
{ 
	...
	while(1) {
		 ...
		 */
		while (PistaLibera == 0) {
			DBGpthread_cond_wait(&cond, &mutex, Plabel);a
			/* VIC: errore grave di logica, la condizione dell'if dovrebbe
				stare in OR LOGICO con la condizione del while. */
			if (numAereiRichiedentiAtterraggio > 0) {
		...
		numAereiRichiedentiAtterraggio++;
